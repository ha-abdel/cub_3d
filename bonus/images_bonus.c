...
/*   Updated: 2025/09/09 09:45:53 by salahian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cube_bonus.h"

void	get_walls_imgs_addresses(t_data **data)
{
...
void	get_imgs_addresses(t_data **data)
{
	...
	(*data)->minimap.addr = mlx_get_data_addr((*data)->minimap.img,
			...
			&(*data)->minimap.endian);
	(*data)->exit.frame_exit.addr = mlx_get_data_addr(
			(*data)->exit.frame_exit.img,
...
void	init_wall_images(t_data **data)
{
	...
	(*data)->w_wall.img = mlx_xpm_file_to_image((*data)->mlx,
			...
			&(*data)->w_wall.height);
	check_walls_addresses(data);
	get_walls_imgs_addresses(data);
}
